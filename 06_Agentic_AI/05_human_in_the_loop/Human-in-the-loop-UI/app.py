...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        type="password",
        help="Leave empty to use GEMINI_API_KEY from your environment or .env file.",
        disabled=st.session_state.proposal is not None,
    )
    env_ready = bool(os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY"))
...
with left:
    ...
    if analyze_clicked:
        if not request_text.strip():
            ...
            st.error("Provide a Gemini API key in the sidebar or set GEMINI_API_KEY.")
        else:
            try:
